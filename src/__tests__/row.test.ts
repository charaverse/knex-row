...
beforeAll(async () => {
  await mysql2Conn.schema.createTable(TABLE, (table) => {
    table.increments("id");

    table.timestamp("time_created").defaultTo(mysql2Conn.fn.now());
    table
      .timestamp("time_updated")
      .defaultTo(
        mysql2Conn.raw("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP")
      );
  ...
  describe("query", () => {
    describe("findAll", () => {
      it("should return array of rows", async () => {
        ...
        expect(
          rows.find((row) => row.id === 1)?.getColumn<string>("name")
        ).toBe("Karlsruhe");
        expect(
          rows.find((row) => row.id === 2)?.getColumn<string>("name")
        ).toBe("Leipzig");
        expect(
          rows.find((row) => row.id === 4)?.getColumn<string>("name")
        ).toBe("Prinz Eugen");
        expect(
          rows.find((row) => row.id === 6)?.getColumn<string>("name")
        ).toBe("Friedrich der Große");
      });
  ...
  describe("row", () => {
    ...
    describe("custom primary key", () => {
      it("should only set specific row with the primary key", async () => {
        const [rowData] = await conn(TABLE).where({
          key: "leipzig",
        });
        const row = new Row({
          conn: conn,
